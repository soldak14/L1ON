...
class AppContext
{
	...
	readonly backend = new BackendService(this.conf, this.model, this.notificationService, this.log);
	readonly world = new WorldService(this.conf, this.model);
	readonly localData = new LocalDataService();
	readonly wallet = new WalletService(this.model, this.world, this.backend, this.localData);
}
