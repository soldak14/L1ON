...
export class WalletService {
    ...
    restoreLogin()
    {
        if (this.localData.wallet !== undefined) {
            ...
            this.model.account.login(this.localData.wallet.address, this.localData.wallet.key, this.localData.wallet.address2, this.localData.wallet.key2, []);
        }
    }
    ...
    async unisatLogin() {
        ...
        if (conf.network !== network)
            await window.unisat.switchNetwork(conf.network === "bitcoin" ? "livenet" : conf.network);
        ...
        if (acc.length) {
            ...
            this.model.account.login(acc[0], key, "", "", inscr.list.map(it => it.inscriptionId));
    ...
    async hiroWalletLogin() {
        ...
        const resolve = async () => {
            ...
            const cardinalAddress = userSession.loadUserData().profile.btcAddress.p2wpkh[network];
            const ordinalsAddress = userSession.loadUserData().profile.btcAddress.p2tr[network];

            const cardinalsPub = userSession.loadUserData().profile.btcPublicKey.p2wpkh;
            const ordinalsPub = userSession.loadUserData().profile.btcPublicKey.p2tr;


            this.model.account.login(cardinalAddress, cardinalsPub, ordinalsAddress, ordinalsPub, []);
    ...
    async xVerseLogin() {
        ...
        const getAddressOptions: GetAddressOptions = {
            payload: {
                ...
                network: {
                    type: conf.network === "bitcoin" ? BitcoinNetworkType.Mainnet : BitcoinNetworkType.Testnet,
                },
            },
            onCancel: () => {
                app.notificationService.showError("Error");
            ...
            onFinish: (response: GetAddressResponse) => {
                let address = response.addresses.find((it: {
                    purpose: string,
                    address: string,
                    publicKey: string
                }) => it.purpose === "payment");

                let p2tr_address = response.addresses.find((it: {
                    purpose: string,
                    address: string,
                    publicKey: string
                }) => it.purpose === "ordinals");

                if (!address && p2tr_address)
                    address = p2tr_address;

                if (!p2tr_address && address)
                    p2tr_address = address;

                if (!address || !p2tr_address)
                    return;

                this.model.account.login(address.address, address.publicKey, p2tr_address.address, p2tr_address.publicKey, []);
    ...
    async ordinalsSafeLogin() {
        ...
        if (conf.network !== network)
            await window.ordinalSafe.switchNetwork(conf.network === "bitcoin" ? "mainnet" : conf.network);
    ...
    async transferInscription(inscriptionId: string, address: string, feeRate?: number): Promise<{
        txId: string
    } | undefined> {
        if (this.currentWallet === WALLETS.unisat) {
            return await window.unisat.sendInscription(address, inscriptionId, feeRate ? {feeRate: feeRate} : undefined);
        }
    }
    ...
    async payBitcoin(toAddress: string, amount: number) {
        ...
        if (this.currentWallet === WALLETS.hiro) {
            return await window.btc.request("sendTransfer", {address: toAddress, amount: amount});
        }
        if (this.currentWallet === WALLETS.xVerse) {

            if (!this.model.btcData.fees)
            ...
            const conf = await this.backend.getConf();
            const realTx = getSendBtcTx(accountAddress, toAddress, amount, publicKey, utxosToUse, this.model.btcData.fees.fastestFee, conf.network === "bitcoin" ? NETWORK : TEST_NETWORK);
            ...
            const signPsbtOptions: SignTransactionOptions = {
                ...
                onFinish: (response: unknown) => {
                    if (response && typeof response === "object" && "psbtBase64" in response) {
    ...
    private setWallet(wallet: WALLETS) {
        ...
        this.localData.wallet = {wallet, address: this.model.account.address, key: this.model.account.pub, address2: this.model.account.p2trAddress, key2: this.model.account.p2tr_pub};
    }
}